...
CharacterPixels getCharacterPixels(char character) {
  ...
  if ('a' <= character && character <= 'z') {
    return getCharacterPixels(character - 'a' + 'A');
  }
  for (size_t i = 0; i < kCharacterMapSize; i++) {
    if (kCharacterMap[i].character == character) {
      return kCharacterMap[i];
    }
  }
  return getCharacterPixels('?');
}

void writeColumn(const CharacterPixels& characterPixels,
                 uint8_t characterColumnFromRight,
                 uint8_t pixelColumnFromRight,
  ...
  while (true) {
    ...
    for (size_t col = col0; col <= col4; col++) {
      if (characterStartColumn <= col && col <= characterEndColumn) {
        ...
        writeColumn(cp, characterColumnFromRight, pixelColumnFromRight,
                    tmpPixels, textColor, backgroundColor);
      }
    }
  }
  for (uint8_t i = 0; i < MATRIX_SIZE; i++) {
