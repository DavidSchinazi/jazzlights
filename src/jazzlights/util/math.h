...
inline uint8_t beatsin8(uint16_t beats_per_minute, uint32_t elapsedTime, uint8_t lowest = 0, uint8_t highest = 255,
...
inline int beatsin(uint16_t beats_per_minute, uint32_t elapsedTime, int lowest, int highest, uint8_t phase_offset = 0) {
  uint8_t beat = beat8(beats_per_minute, elapsedTime);
  uint8_t beatsin = sin8(beat + phase_offset);
