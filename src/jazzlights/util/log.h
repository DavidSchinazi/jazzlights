...
inline void enable_debug_logging() {
  gDebugLoggingEnabled = true;
}
...
#define _LOG_AT_LEVEL(levelStr, format, ...) \
  arduinoLog(levelStr ": " format, ##__VA_ARGS__)

#else  // ARDUINO

#define _LOG_AT_LEVEL(levelStr, format, ...) \
  printf(levelStr ": " format "\n", ##__VA_ARGS__)
...
#define debug(format, ...) \
  do { \
    if (is_debug_logging_enabled()) { \
      _LOG_AT_LEVEL("DEBUG", format, ##__VA_ARGS__); \
    } \
  } while (0)
