...
static int8_t inline __attribute__((always_inline))
selectBasedOnHashBit(uint8_t hash, uint8_t bitnumber, int8_t a, int8_t b) {
  int8_t result;
#if !defined(__AVR__)
  result = (hash & (1 << bitnumber)) ? a : b;
#else
...
#ifdef FADE_12
uint16_t logfade12(uint16_t val) { return scale16(val, val) >> 4; }
...
uint16_t inoise16(uint32_t x) { return ((uint32_t)((int32_t)inoise16_raw(x) + 17308L)) << 1; }
