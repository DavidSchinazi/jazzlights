...
Max485BusHandler::Max485BusHandler(uart_port_t uartPort, int txPin, int rxPin, BusId bus_id)
    : uartPort_(uartPort), txPin_(txPin), rxPin_(rxPin), bus_id_(bus_id) {
  ...
  if (sharedSendBuffer_ == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler shared send buffer size %d", kUartBufferSize);
  }
  taskSendBuffer_ = reinterpret_cast<uint8_t*>(malloc(kUartBufferSize));
  if (taskSendBuffer_ == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler task send buffer size %d", kUartBufferSize);
  }
  taskRecvBuffer_ = reinterpret_cast<uint8_t*>(malloc(kUartBufferSize));
  if (taskRecvBuffer_ == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler task recv buffer size %d", kUartBufferSize);
  }
  sharedRecvBuffer_ = reinterpret_cast<uint8_t*>(malloc(kUartBufferSize));
  if (sharedRecvBuffer_ == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler shared recv buffer size %d", kUartBufferSize);
  }
  encodedReadBuffer_ = reinterpret_cast<uint8_t*>(malloc(kUartBufferSize));
  if (encodedReadBuffer_ == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler encoded read buffer size %d", kUartBufferSize);
  }
  decodedReadBuffer_ = reinterpret_cast<uint8_t*>(malloc(kUartBufferSize));
  if (decodedReadBuffer_ == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler decoded read buffer size %d", kUartBufferSize);
  }
}
...
void Max485BusHandler::WriteMessage(const uint8_t* buffer, size_t length, BusId destBusId) {
  ...
  jll_uart_data_buffer(uartBuffer2, uartBufferIndex2, "Max485BusHandler writing");
  return WriteData(uartBuffer2, uartBufferIndex2);
}

size_t Max485BusHandler::DecodeMessage(const uint8_t* encodedBuffer, size_t encodedLength, uint8_t* decodedBuffer,
                                       size_t maxDecodedLength) {
  jll_uart_data_buffer(encodedBuffer, encodedLength, "DecodeMessage attempting to parse");
  if (encodedLength < ComputeExpansion(0)) {
    jll_buffer_error(encodedBuffer, encodedLength, "Max485BusHandler DecodeMessage ignoring very short message");
    return 0;
  }
  ...
  if (separator != kSeparator) {
    jll_buffer_error(encodedBuffer, encodedLength,
                     "Max485BusHandler DecodeMessage ignoring message due to bad initial separator");
    return 0;
  }
  BusId destBusId = encodedBuffer[uartBufferIndex++];
  if (destBusId != kBusIdBroadcast && destBusId != kBusId) {
    jll_buffer_error(encodedBuffer, encodedLength, "Max485BusHandler DecodeMessage ignoring message not meant for us");
    return 0;
  }
  if (encodedBuffer[encodedLength - 2] != kSeparator) {
    jll_buffer_error(encodedBuffer, encodedLength, "Max485BusHandler DecodeMessage did not find end separator");
    return 0;
  }
  if (encodedBuffer[encodedLength - 1] != kBusIdEndOfMessage) {
    jll_buffer_error(encodedBuffer, encodedLength, "Max485BusHandler DecodeMessage did not find end of message");
    return 0;
  }
  ...
  if (decodeLength == 0) {
    jll_buffer_error(encodedBuffer, encodedLength,
                     "Max485BusHandler DecodeMessage failed to CobsDecode incoming message");
    return 0;
  }
...
size_t Max485BusHandler::ReadMessage(uint8_t* buffer, size_t maxLength, BusId* outDestBusId) {
  size_t readLength = ReadData(encodedReadBuffer_ + encodedReadBufferIndex_, kUartBufferSize - encodedReadBufferIndex_);
  if (readLength == 0) { return 0; }
  jll_uart_data_buffer(encodedReadBuffer_ + encodedReadBufferIndex_, readLength, "Max485BusHandler read");
  encodedReadBufferIndex_ += readLength;
  jll_uart_data_buffer(encodedReadBuffer_, encodedReadBufferIndex_, "Max485BusHandler data we now have");
...
void SetupUart(Milliseconds currentTime) { (void)Max485BusHandler::Get(); }
...
void RunUart(Milliseconds currentTime) {
  ...
  if (outerRecvBuffer == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler outer recv buffer size %d", kUartBufferSize);
  }
  static uint8_t* outerSendBuffer = reinterpret_cast<uint8_t*>(malloc(kUartBufferSize));
  if (outerSendBuffer == nullptr) {
    jll_fatal("Failed to allocate Max485BusHandler outer send buffer size %d", kUartBufferSize);
  }
  BusId destBusId = kSeparator;
  size_t readLength = Max485BusHandler::Get()->ReadMessage(outerRecvBuffer, kUartBufferSize, &destBusId);
  ...
  if (destBusId == kBusId) {
    ...
    Max485BusHandler::Get()->WriteMessage(outerSendBuffer, bytesWritten, kBusIdLeader);
  }
#else   // L_BUS_LEADER
  ...
  Max485BusHandler::Get()->WriteMessage(outerSendBuffer, bytesWritten, kFollowerBusIds[sCurrentFollower]);
