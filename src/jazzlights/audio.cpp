...
#ifndef JL_M5_USE_INTERNAL_MICROPHONE
#define JL_M5_USE_INTERNAL_MICROPHONE 0
#endif

#if (JL_IS_CONTROLLER(CORES3) && !JL_M5_USE_INTERNAL_MICROPHONE) || JL_IS_CONTROLLER(CORE2AWS)
...
void Audio::Initialize() {
  ...
  // cfg.dma_buf_count = 3;
  cfg.dma_buf_len = static_cast<size_t>(kFFTSize);
  // cfg.over_sampling = 1;
  cfg.noise_filter_level = 16;

  cfg.sample_rate = kSampleRate;
  cfg.magnification = 16;  // cfg.use_adc ? 16 : 1;
  M5.Mic.config(cfg);
  ...
  //   if(false)
  //   {
  //     # if !(JL_IS_CONTROLLER(CORES3) || JL_IS_CONTROLLER(M5STICK_C))
  // #error "Internal microphone is only supported on CoreS3 and StickC"
  // # endif
  ...
  //         },
  //     };
  //     jll_info("Initializing I2S in standard mode with internal microphone...");
  //     ESP_ERROR_CHECK(i2s_channel_init_std_mode(rx_handle_, &std_cfg));
  //     jll_info("Initialized I2S in standard mode with internal microphone...");

  //     // Prevent M5Unified from initializing I2S and starting a conflicting task.
  //     auto mic_cfg = M5.Mic.config();
  //     mic_cfg.pin_data_in = -1;
  //     M5.Mic.config(mic_cfg);
  //     M5.Mic.begin();

  // // 2. CoreS3 (External PCM1808 Mic) OR Core2AWS (Philips Slot, 32-bit, External Pins)
  // #elif JL_IS_CONTROLLER(CORES3) || JL_IS_CONTROLLER(CORE2AWS)

  //     i2s_std_config_t std_cfg = {
  //         .clk_cfg  = I2S_STD_CLK_DEFAULT_CONFIG(kSampleRate),
  //         .slot_cfg = I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO),
  //         .gpio_cfg = {
  //             .mclk = static_cast<gpio_num_t>(kPinE1_3),
  //             .bclk = static_cast<gpio_num_t>(kPinC1),
  //             .ws   = static_cast<gpio_num_t>(kPinE2_2),
  //             .dout = I2S_GPIO_UNUSED,
  //             .din  = static_cast<gpio_num_t>(kPinC2),
  //             .invert_flags = {.mclk_inv = false, .bclk_inv = false, .ws_inv = false},
  //         },
  //     };
  ...
  // // 3. Atom Matrix OR Atom S3 (PDM Mode, Mono)
  // #elif JL_IS_CONTROLLER(ATOM_MATRIX) || JL_IS_CONTROLLER(ATOM_S3)

  //     #if JL_IS_CONTROLLER(ATOM_MATRIX)
  //         gpio_num_t clk_pin = GPIO_NUM_22;
  //         gpio_num_t din_pin = GPIO_NUM_19;
  //     #elif JL_IS_CONTROLLER(ATOM_S3) // (Assuming S3U)
  //         gpio_num_t clk_pin = GPIO_NUM_41;
  //         gpio_num_t din_pin = GPIO_NUM_42;
  //     #elif JL_IS_CONTROLLER(M5STICK_C)
  //         gpio_num_t clk_pin = GPIO_NUM_0;
  //         gpio_num_t din_pin = GPIO_NUM_34;
  //     #endif
  ...
  // #endif
  //   }
  jll_info("I2S microphone started");
...
void Audio::ReadAndProcessAudio() {
  ...
  M5.Mic.record(audio_buffer_, static_cast<size_t>(kFFTSize), kSampleRate, false);
  ...
  for (int i = 0; i < kFFTSize; i++) {
    fft_input_[i * 2] = (float)audio_buffer_[i];
