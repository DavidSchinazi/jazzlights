...
bool discoveryResultsMatch(const PixelPusherDiscoveryResult& a, const PixelPusherDiscoveryResult& b) {
  return memcmp(&a.addressAndPort.sin_addr, &b.addressAndPort.sin_addr, sizeof(a.addressAndPort.sin_addr)) == 0 &&
         a.addressAndPort.sin_port == b.addressAndPort.sin_port && a.controller == b.controller && a.group == b.group;
}
...
class PixelPusherDiscovery {
  ...
  std::list<const struct sockaddr_in*> getAddresses(struct sockaddr_in* defaultAddressAndPort, int32_t controller,
    ...
    if (controller != 0 && group != 0) {
      for (const PixelPusherDiscoveryResult& r : results_) {
        if (r.controller != controller || r.group != group) { continue; }
        bool alreadyInList = false;
        for (const struct sockaddr_in* sin : results) {
          if (memcmp(&r.addressAndPort.sin_addr, &sin->sin_addr, sizeof(sin->sin_addr)) == 0 &&
 ...
 private:
  static uint16_t readUint16LE(const uint8_t* data) { return data[0] | (data[1] << 8); }
  ...
  void receiveLoop(Milliseconds currentTime) {
    ...
    while (true) {
      ...
      static constexpr size_t kPortOffset = 6 + 4 + 1 + 1 + 2 + 2 + 2 + 2 + 4 + 1 + 1 + 2 + 4 + 4 + 4 + 4 + 4 + 2 + 2;
      static constexpr size_t kControllerOffset = 6 + 4 + 1 + 1 + 2 + 2 + 2 + 2 + 4 + 1 + 1 + 2 + 4 + 4 + 4;
            ...
            readUint32LE(&buffer[kGroupOffset]), srcAddrStr, ntohs(srcAddr.sin_port));
      {
        ...
        memcpy(&result.addressAndPort.sin_addr, &buffer[kIpAddressOffset], sizeof(result.addressAndPort.sin_addr));
      ...
      }
      {
        ...
        memcpy(&result2.addressAndPort.sin_addr, &(srcAddr.sin_addr), sizeof(result2.addressAndPort.sin_addr));
  ...
  void pruneResults(Milliseconds currentTime) {
    ...
    if (currentTime <= kMaxLifeTime) { return; }
...
void PixelPusher::render(InputStream<Color>& pixelColors) {
  ...
  if (lastTxTime > 0 && currTime - lastTxTime < throttle) { return; }
  lastTxTime = currTime;
...
#endif  // REDUCE_PIXELPUSHER_BRIGHTNESS
    pxcnt++;
    if (pxcnt > HACKY_HARD_CODED_NUM_OF_LEDS) { break; }
  }
